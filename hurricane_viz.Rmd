...
strm_lst <- storms %>% group_by(Name) %>% summarise(max=max(as.numeric(cat)))

strm2 <- strm_lst %>% group_by(max) %>% summarise(count = n())

strm2$category <- strm2$max-1
strm2

ggplot(strm2, aes(category,count)) + geom_bar(stat = 'Identity') + geom_text(stat='Identity',aes(label=`count`), vjust = -0.6)
